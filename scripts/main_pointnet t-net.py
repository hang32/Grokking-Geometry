...
from pointnet_scaling_experiment.model1 import PointNetTNet 
...
def plot_avg_s_curve(sizes, std_means, std_stds, ood_means, ood_stds):
    ...
    plt.plot(sizes, ood_means, marker='s', linestyle='--', color='tab:red', label='OOD Test (Mean)', linewidth=2)
    plt.fill_between(sizes, ood_means - ood_stds, ood_means + ood_stds, color='tab:red', alpha=0.2)
    
    plt.xscale('log')
    plt.xlabel('Training Data Size (N)', fontsize=12)
    plt.ylabel('Test Accuracy', fontsize=12)
    plt.title('Scaling Law: PointNet + T-Net (Mean $\pm$ Std)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('results', 's_curve_avg.png'))
    print("Average S-Curve updated: results/s_curve_avg.png")
...
def train_single_run(size, seed, train_x, train_y, test_std_loader, test_ood_loader):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for inputs, labels in train_loader:
            ...
            loss = criterion(outputs, labels) + 0.01 * feature_transform_regularizer(trans)
...
def main():
    ...
    print(f">>> Starting Average Experiment (T-Net Reg=0.1)")
    ...
    for i, size in enumerate(DATA_SIZES):
        ...
        torch.save(best_run['state_dict'], f'checkpoints/PointNetTNet_n{size}_avg_best.pth')
        ...
        plot_learning_dynamics(
            ...
            f"T-Net N={size} (Best Seed)",
            f"dynamics_tnet_avg_n{size}.png"
        )
    ...
    print("\nAll Done! Result saved to results/s_curve_avg.png")
