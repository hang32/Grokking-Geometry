...
def drop_and_resample(pc_batch, drop_ratio=0.05):
   
    B, N, C = pc_batch.shape
    keep_n = int(N * (1 - drop_ratio))
    resampled_pc = np.zeros_like(pc_batch)
    
    for i in range(B):
      
        kept_indices = np.random.choice(N, keep_n, replace=False)
        kept_points = pc_batch[i, kept_indices, :]
        
        
        resample_indices = np.random.choice(keep_n, N, replace=True)
        resampled_pc[i] = kept_points[resample_indices, :]
        
    return resampled_pc.astype(np.float32)
...
def plot_learning_dynamics(train_losses, train_accs, test_std_accs, test_ood_accs, title, filename):
    ...
    ax2.plot(epochs, test_std_accs, 'g-', label=f'Test Std Acc (Drop {int(DROP_RATIO*100)}%)')
    ax2.plot(epochs, test_ood_accs, 'r-', label=f'Test OOD Acc (Drop {int(DROP_RATIO*100)}%)', linewidth=2)
...
def plot_avg_s_curve(sizes, std_means, std_stds, ood_means, ood_stds):
    ...
    plt.plot(sizes, std_means, marker='o', linestyle='-', color='tab:blue', label=f'Standard Test (Drop {int(DROP_RATIO*100)}%)', linewidth=2)
    plt.fill_between(sizes, std_means - std_stds, std_means + std_stds, color='tab:blue', alpha=0.2)
    
    # 用橙色线表示点云采样测试
    plt.plot(sizes, ood_means, marker='v', linestyle='--', color='darkorange', label=f'OOD Test (Drop {int(DROP_RATIO*100)}%)', linewidth=2.5, markersize=8)
    plt.fill_between(sizes, ood_means - ood_stds, ood_means + ood_stds, color='darkorange', alpha=0.2)
    
    plt.xscale('log')
    plt.xlabel('Training Data Size (N)', fontsize=12)
    plt.ylabel('Test Accuracy', fontsize=12)
    plt.title(f'Scaling Law: PointNet Robustness (Random Drop {int(DROP_RATIO*100)}%)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('results', f's_curve_simple_drop{DROP_RATIO}.png'))
...
def main():
    ...
    print(f"\n[WARNING] Injecting Point Drop & Resample (ratio={DROP_RATIO}) to TEST sets strictly!")
    test_std_x = drop_and_resample(test_std_x, drop_ratio=DROP_RATIO)
    test_ood_x = drop_and_resample(test_ood_x, drop_ratio=DROP_RATIO)
    ...
    print(f">>> Starting Experiment: Simple PointNet (Drop Points Test)")
    for i, size in enumerate(DATA_SIZES):
        ...
        torch.save(best_run['state_dict'], f'checkpoints/SimplePointNet_n{size}_drop{DROP_RATIO}_best.pth')
        
        plot_learning_dynamics(best_run['history']['train_loss'], best_run['history']['train_acc'], best_run['history']['test_std_acc'], best_run['history']['test_ood_acc'], f"Simple PointNet Drop N={size}", f"dynamics_simple_drop{DROP_RATIO}_n{size}.png")
        
        plot_avg_s_curve(DATA_SIZES[:len(ood_means)], std_means, std_stds, ood_means, ood_stds)
        
    print(f"\nAll Done! Result saved to results/s_curve_simple_drop{DROP_RATIO}.png")
