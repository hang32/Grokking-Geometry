...
from pointnet_scaling_experiment.models.model import PointTransformer 
...
def plot_learning_dynamics(train_losses, train_accs, test_std_accs, test_ood_accs, title, filename):
    ...
    ax2.plot(epochs, test_std_accs, 'g-', label='Test Std Acc')
    ax2.plot(epochs, test_ood_accs, 'r-', label='Test OOD Acc', linewidth=2)
...
def plot_avg_s_curve(sizes, std_means, std_stds, ood_means, ood_stds):
    ...
    plt.plot(sizes, std_means, marker='o', linestyle='-', color='tab:blue', label='Standard Test (Mean)', linewidth=2)
    plt.fill_between(sizes, std_means - std_stds, std_means + std_stds, color='tab:blue', alpha=0.2)
    

    plt.plot(sizes, ood_means, marker='*', linestyle='--', color='tab:green', label='PT OOD Test (Mean)', linewidth=2, markersize=10)
    plt.fill_between(sizes, ood_means - ood_stds, ood_means + ood_stds, color='tab:green', alpha=0.2)
    
    plt.xscale('log')
    plt.xlabel('Training Data Size (N)', fontsize=12)
    plt.ylabel('Test Accuracy', fontsize=12)
    plt.title('Scaling Law: Point Transformer (Self-Attention)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('results', 's_curve_pt_noise0.05_avg.png'))
    print("Average S-Curve updated: results/s_curve_pt_noise0.05_avg.png")
...
def load_data():
    ...
    try:
        ...
        test_std_x = np.load('data/test_std_x.npy')
        test_std_y = np.load('data/test_std_y.npy')
        test_ood_x = np.load('data/test_ood_x.npy')
        test_ood_y = np.load('data/test_ood_y.npy')
...
def train_single_run(size, seed, train_x, train_y, test_std_loader, test_ood_loader):
    ...
    curr_batch_size = min(BATCH_SIZE, size)
    train_loader = DataLoader(train_dataset, batch_size=curr_batch_size, shuffle=True)
    
  
    model = PointTransformer(k=16).to(DEVICE)
    ...
    return {
        ...
        'final_std_acc': best_std_acc,
        'history': history,
        'state_dict': best_state_dict
    }

def main():
    if not os.path.exists('data/train_x.npy'):
    ...
    NOISE_STD = 0.05  # 噪声强度。你可以尝试 0.02 (轻微), 0.05 (中等), 0.10 (严重)
    ...
    print(f">>> Starting Experiment: Point Transformer")
    ...
    for i, size in enumerate(DATA_SIZES):
        ...
        ood_mean = np.mean(curr_ood_accs)
        ood_std = np.std(curr_ood_accs)
        std_mean = np.mean(curr_std_accs)
        std_std = np.std(curr_std_accs)
        ...
        ood_stds.append(ood_std)
        
        best_run = sorted(runs, key=lambda x: x['final_ood_acc'], reverse=True)[0]
        torch.save(best_run['state_dict'], f'checkpoints/PointTransformer_n{size}_best.pth')
        
        plot_learning_dynamics(
            best_run['history']['train_loss'],
            best_run['history']['train_acc'],
            best_run['history']['test_std_acc'],
            best_run['history']['test_ood_acc'],
            f"Point Transformer N={size}",
            f"dynamics_pt_n{size}.png"
        )
        
        current_sizes = DATA_SIZES[:len(ood_means)]
        plot_avg_s_curve(current_sizes, std_means, std_stds, ood_means, ood_stds)
        
    print("\nAll Done! Result saved to results/s_curve_pt_avg.png")
