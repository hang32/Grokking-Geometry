...
from pointnet_scaling_experiment.model1 import DGCNN 
...
def plot_avg_s_curve(sizes, std_means, std_stds, ood_means, ood_stds):
    ...
    # 2. OOD Test (红色)
    plt.plot(sizes, ood_means, marker='^', linestyle='--', color='tab:red', label='DGCNN OOD Test (Mean)', linewidth=2)
    ...
    plt.title('Scaling Law: DGCNN (Dynamic Graph CNN)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('results', 's_curve_dgcnn_avg.png'))
    print("Average S-Curve updated: results/s_curve_dgcnn_avg.png")
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for inputs, labels in loader:
            ...
            outputs = model(inputs) # DGCNN 只返回 outputs
...
def main():
    ...
    print(f">>> Starting Experiment: DGCNN Only")
    ...
    for i, size in enumerate(DATA_SIZES):
        ...
        torch.save(best_run['state_dict'], f'checkpoints/DGCNN_n{size}_best.pth')
