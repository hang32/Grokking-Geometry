...
from pointnet_scaling_experiment.models.model import VNPointNet 
...
def plot_avg_s_curve(sizes, std_means, std_stds, ood_means, ood_stds):
    ...
    plt.plot(sizes, ood_means, marker='D', linestyle='-', color='goldenrod', label='VN-PointNet OOD (Mean)', linewidth=2.5, markersize=8)
    plt.fill_between(sizes, ood_means - ood_stds, ood_means + ood_stds, color='goldenrod', alpha=0.2)
    
    plt.xscale('log')
    plt.xlabel('Training Data Size (N)', fontsize=12)
    plt.ylabel('Test Accuracy', fontsize=12)
    plt.title('Scaling Law: Vector Neurons (Perfect Rotation Invariance)', fontsize=14)
    plt.grid(True, which="both", ls="-", alpha=0.4)
    plt.xticks(sizes, labels=[str(s) for s in sizes], rotation=45)
    plt.ylim(0, 1.05)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join('results', 's_curve_vn_avg.png'))
...
def main():
    ...
    print(f">>> Starting Experiment: VN-PointNet (Vector Neurons)")
    for i, size in enumerate(DATA_SIZES):
        ...
        torch.save(best_run['state_dict'], f'checkpoints/VNPointNet_n{size}_best.pth')
        
        plot_learning_dynamics(best_run['history']['train_loss'], best_run['history']['train_acc'], best_run['history']['test_std_acc'], best_run['history']['test_ood_acc'], f"VN-PointNet N={size}", f"dynamics_vn_n{size}.png")
        plot_avg_s_curve(DATA_SIZES[:len(ood_means)], std_means, std_stds, ood_means, ood_stds)
        
    print("\nAll Done! Result saved to results/s_curve_vn_avg.png")
